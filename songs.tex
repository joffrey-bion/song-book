...
\usepackage{slashbox} % pour les cellules de tableaux coupees en diagonale
\usepackage{xargs} % pour les macros x a plusieurs arguments optionnels
\usepackage{multicol} % pour les environnement a plusieurs colonnes
\usepackage{makeidx} % pour faire un index si besoin
...
\usepackage{hyperref} % pour transformer toutes les refs en liens (\ref, toc, index...)
\usepackage{xcolor}
...
\renewcommand\headrulewidth{0pt} % supprime la ligne sous le header
\pagestyle{fancy}

% --- Macros et environnements specifiques -------------------------------------
...
\newcommand{\madate}{\the\year} % \today pour mettre la date a la compilation
...
% Titres : 
%    Premiere lettre du titre en majuscule
%    Premiere lettre de chaque mot en majuscule (sauf le, la, de, des, the, of...)
